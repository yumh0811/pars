...
#输入csv
file_SNPs_PARS_syn <- paste0(path,'/data_SNPs_PARS_syn.update.csv',collapse = NULL)
...
#输入csv
file_SNPs_PARS_nsy <- paste0(path,'/data_SNPs_PARS_nsy.update.csv',collapse = NULL)
data_SNPs_PARS_nsy <- read.csv(file_SNPs_PARS_nsy,header = TRUE,sep = ",")
dd_SNP <- data.frame(name = "nsy",SNP = c(nrow(data_SNPs_PARS_nsy)))
data_gene_process <- data_SNPs_PARS_nsy["gene"]
data_gene_process <- unique(data_gene_process,fromLast=TRUE)
dd_gene<- data.frame(name = "nsy", gene = c(nrow(data_gene_process)))
