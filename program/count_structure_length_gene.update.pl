...
=head1 SYNOPSIS
    ...
    perl ~/Scripts/pars/program/count_structure_length_gene.update.pl --file ~/data/mrna-structure/process/$NAME.gene_variation.process.update.yml --name ~/data/mrna-structure/result/${NAME}/mRNA.gene.list.csv --structure stem --output stem_length_mRNA.update.csv
    perl ~/Scripts/pars/program/count_structure_length_gene.update.pl --file ~/data/mrna-structure/process/$NAME.gene_variation.process.update.yml --name ~/data/mrna-structure/result/${NAME}/mRNA.gene.list.csv --structure loop --output loop_length_mRNA.update.csv
    ...
    if ( $snp[0] eq "gene" ) {
        ...
        if ( $structure eq "stem" ) {
            ...
            {    # $snp[0] represents snp in which gene
                ...
                my $loop = AlignDB::IntSpan->new( $info->{loop} );
